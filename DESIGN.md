...
## Mimari Özeti (Executive Summary)

Sistemin üst düzey çalışma mantığı aşağıdaki gibidir:

```mermaid
graph TD
    User -->|API| Gateway[API Gateway / Ingestion]
    Gateway --> Processor[TreeChunker Processor]
    Processor --> VectorStore[FAISS Vector Store]
    User -->|Query| Agent[Document Analyst Agent]
    Agent --> Tools[Search/Extract Tools]
    Tools --> VectorStore
    Agent --> Validator[Validator Node]
    Validator -->|Retry| Agent
    Validator -->|Success| Response[Final Response]
```
...
- **Not:** Bellek yönetimi mimarimiz, gizlilik prensipleri gereği oturum sonlandığında `AgentState` verisinin temizlenmesini (ephemeral storage) destekleyecek şekilde tasarlanmıştır. Kalıcı belleğe geçişte (Cross-session memory), verinin *encryption-at-rest* ile korunması Enterprise güvenlik standartlarımızın temelini oluşturacaktır.
